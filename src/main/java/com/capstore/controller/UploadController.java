...
public class UploadController {
	...
	public ResponseEntity<List<String>> getListFiles(Model model,@PathVariable("productId") String productId) {
		...
		List<String> fileNames = files
				.stream().map(fileName -> MvcUriComponentsBuilder
						.fromMethodName(UploadController.class, "getFile", productId+".jpg").build().toString())
	...
	public ResponseEntity<List<ProductImage>> addImage(@RequestBody ProductImage productImage)
	{
		List<ProductImage> productImages=storageService.dbStore(productImage);
		if (productImages.isEmpty())
			return new ResponseEntity("Sorry!Image not found!", HttpStatus.NOT_FOUND);
